  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        PGDATA = "./.pgdata";
        PGPORT = "5432";
        DB_USER = "postgres"; # Must be lowercase
        DB_PASSWORD = "postgres"; # Must be lowercase
      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
              ...
              find_available_port() {
                ...
                local max_port=5442
                local port_file="$PGDATA/.pgport"

                if [ -f "$port_file" ]; then
                  local saved_port=$(cat "$port_file")
                  if [ "$saved_port" -ge "$base_port" ] && [ "$saved_port" -le "$max_port" ]; then
                    if ! ss -tln 2>/dev/null | grep -q ":$saved_port "; then
                      echo "$saved_port"
                      return
                    fi
                  fi
                ...
                echo "Error: No available port found in range $base_port-$max_port" >&2
                exit 1
              }
            ...
            EOF
              ...
              trap 'if pg_ctl status > /dev/null 2>&1; then echo "Stopping PostgreSQL..."; pg_ctl stop; fi' EXIT
          '';
        };
        ...
        apps = {
          stop = {
            type = "app";
            program = toString (
              pkgs.writeShellScript "stop-postgres" ''
                export PGDATA=${PGDATA}
          ...
          reset = {
            type = "app";
            program = toString (
              pkgs.writeShellScript "reset-postgres" ''
                export PGDATA=${PGDATA}
                export PGRUNDIR=${PGRUNDIR}
